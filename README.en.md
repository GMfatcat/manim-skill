...
The render backend renders each beat independently in its own sandboxed container, in parallel, with graceful per-beat / per-clip failure handling. It also tags each beat with the cost tier that resolved it and writes a cost-tier `summary` (escalation rate + free-tier rate) into the manifest. The service backend turns that into an async job API; the Streamlit frontend and the CLI's remote mode are both thin clients of it. The whole thing deploys as one universal Docker image via docker-compose.
...
`scripts/eval/run_smoke.py` points `OpenAIClient` at any OpenAI-compatible endpoint (here: OpenRouter free models) and exercises the LLM half — `analyze` / `codegen` / full pipeline — on the materials in `tests/realworld-test/` (a code snippet; the paper/report inputs aren't shipped with the repo). Two stages report per-concept success and dump the validated specs; `scripts/eval/render_specs.py` then renders them and reports per-beat results. `scripts/eval/bundle_specs.py` takes any directory of validated specs and renders them as one `render_batch`, producing a single zip + `manifest.json` — the natural end-to-end deliverable; pass `--repair --model <slug>` to re-ask the LLM to fix `raw` beats that fail (pair with `--max-workers 1` on a rate-limited free endpoint), and `--escalation-quota <frac>` to warn when too many beats stay unresolved. The bundler and the CLI `bundle` both print a **cost-tier distribution** — how many beats resolved deterministically (component) / by generation / by repair vs. stayed unresolved — and embed it in `manifest.json` as a `summary`. That is the measurement layer of the cost-cascade framework (see `docs/superpowers/specs/2026-06-16-agent-openmodel-cost-cascade-design.md`). Each unresolved beat's detail is also recorded in the manifest, and `manim-skill backflow <dirs/zips>` clusters those across runs by keyword into a markdown "candidate components to add" report — the framework's *what to add* signal once escalation runs high (`docs/superpowers/specs/2026-06-17-contract-gap-backflow-design.md`).
